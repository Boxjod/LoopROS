...
2026-09-07：用户明确提交的 Task 默认增加文件读取/搜索、带备份的修改、Python 语法检查和哈希绑定执行，以及 Node 管理工具。它们继续经过现有 PermissionGate；`run_python=ask/deny` 不会因为后台执行而自动变成 allow。定时／事件任务不开放这些写入、进程控制和任意 Python 执行能力。已有用户策略优先于发行默认值，旧监督器需重新启动才加载新策略；不会自动恢复等待中的机器人任务。

Worker 可在缺少初始 checks 时通过 `task_feedback.checks` 提出验收项，监督器验证格式和工具范围后一次性写入任务。已有 checks 不能被 Worker 替换；新 checks 同样要通过实际工具回执。代码修改使用原有 `edit_file/write_file` 的备份与 diff，`run_python` 需要读取后的当前哈希。任意脚本自行修改外部文件不受这套文件备份机制覆盖。

运动闭环的验收应关联设备身份、命令与新鲜关节反馈；“机械臂动了”不等于到达目标或安全完成。力矩超限、反馈失效和停止失败必须由机器人本地控制器处理，不能等待模型下一轮判断。需提供每关节阈值、单位/标定、采样有效期、停止接口和停止回执；电流/原始负载不能未经标定当作 Nm。本轮没有接入 LoopMaster 实机力矩保护，也没有执行动作测试；这些条件缺失时仍应阻止自主运动。

- `/task 目标`：直接提交持久任务；无 checks 可先由 Worker 收集信息，并通过 task_feedback.checks 提出从目标推导的验收项，监督器仍按实际工具回执判定。无法确定验收条件时等待补充，不能据文字自动完成。
- `task_submit(goal, checks)`：Master 使用的结构化提交工具。
- `/tasks`：查看当前会话任务；`/tasks all` 显式查看所有会话及无归属历史任务；`/tasks status ID`：查看逐步反馈。
...
任务归属（2026-09-07）：tasks 表新增可空 session_id 索引列，手动提交由宿主填写，模型参数不能伪造归属。既有任务和全局调度任务保持无归属，不进入左右键列表；历史回执保留。状态反馈通知和已读游标也按会话隔离，其他会话的后台任务继续按原状态运行，切换面板或 `/new` 不取消它们。`/resume SESSION_ID` 才恢复原会话及其任务范围。
