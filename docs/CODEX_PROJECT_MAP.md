...
| 介绍网页／一行下载安装 | [网页](../website/index.html)、[Shell](../install.sh)、[PowerShell](../install.ps1)、[安装说明](INSTALL.md) | Shell --check、JS语法／交互检查；Windows实机待验证 |
| Loop ROS 品牌／loop ros／旧命令兼容 | [package](../pyproject.toml)、[installer](../install.py)、[App](../terminal/app.py) | test_branding＋项目外启动 |
...
| 安装与全局命令／欢迎页 | [install](../install.py)、[package](../pyproject.toml)、[launcher](../launcher.py)、[UI](../terminal/ui.py)、[安装说明](INSTALL.md) | test_ui＋项目外 loop --version＋PTY |
...
| Qwen API 与工具循环 | [llm](../terminal/llm.py)、[配置](../config.example.json) | API 替身测试；线上未测 |
...
| Master／子 Agent 进程与通信 | [AgentRuntime](../terminal/agents.py)、[角色定义](../agents.json)、[规范](AGENT_RUNTIME.md) | test_agents |
...
| MuJoCo 物理执行／reset | [MujocoBody](../toolchain/mujoco_sim.py)、[demo](../run_sim.py) | 可选环境 unittest＋run_sim |
...
| 运行／检查反馈 | [RUNBOOK](RUNBOOK.md)、[demo](../run_demo.py) | 实际退出码与 review |
...
| 未完成任务持续迭代／后台监督／定时与OS事件 | [任务账本](../core/tasks.py)、[监督器](../terminal/task_supervisor.py)、[后台进程](../terminal/task_service.py)、[配置](../task_runtime.json)、[使用说明](TASK_RUNTIME.md) | test_task_supervisor；真实独立进程、显式提交；历史用户模型只读验收 |
...
| GitHub 初版准备／提示词收敛／GPT-6自定义API | [发布准备](GITHUB_RELEASE.md)、[配置](QUICK_SETUP.md)、[基础上下文](../terminal/conversation_context.py)、[Changelog](../CHANGELOG.md) | test_setup、test_coding_agent、test_harness_behavior；实际GPT-6待验证 |
