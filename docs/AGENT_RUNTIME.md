...
消息通过宿主 broker 和 Pipe 传递；sender 由宿主绑定，子 Agent 无法冒充 Master。每任务最多16条、每条2000字符，在下一次模型调用边界读取；排队确认不是已读取保证。运行中可追加，已终止任务不可续写，需要新任务携带必要结果重新委派。每个子 Agent 获得宿主分配的 agent_id，并可调用 agents_status 获取 self_id 与运行中 peers（ID、角色、状态）；用 send_agent 向同伴发消息。消息参数只接受 agent_id/message，发送者由 broker 绑定，不能再 spawn 或取消同伴。Master 与子 Agent 的消息/查询入口均经过同一权限门禁；plan 模式禁止新建子 Agent 和发送补充任务消息。
...
`~/.loop/agents.json` 是用户可信的声明式扩展入口（可由 `LOOP_HOME` 指定目录）；不存在时加载发行默认值 `configs/agents.json`。用户文件整体替代默认角色表，每个角色具有 provider、tools、prompt；provider 限 llm/expert，子任务可授予的执行工具上限当前为 run_sim/generate_scene。新增角色在下次启动加载。配置不能包含可执行 Python，不能注册 shell、设备控制或递归 spawn。
...
Master 按需 `load_toolset(name="agents")` 后，可以连续调用多个 `spawn_agent`；启动返回即代表进程已创建，不等待前一个任务完成。`agents_status` 返回 max_workers、running 和任务状态。当前默认最多 3 个并行子 Agent；满额时明确拒绝新任务，释放槽位后可再提交，没有隐式等待队列或 DAG 调度器。启动新任务前回收已到达的完成回执，避免完成任务占用旧槽位。
