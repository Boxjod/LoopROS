...
## 13. 2026-09-08 工具调用过多的代码与日志诊断

问题：Loop ROS 为何比用户使用 Codex 时出现更多工具调用。本轮只诊断，没有修改执行器或运行机器人。

样本：本地 conversation.sqlite 中按 id 倒序最近 600 条 tool/result 事件，共 300 次 tool 调用；不是同一任务或同一轮。read_file 113、skill_read 36，合计149（49.7%）；session_task_update 30（10%）；list_files25、run_python17、node_status16、skill_executables13。149次读取中有68种完全相同的调用文本，重复超出首次的调用81次；最常见读取文本出现12次。跨会话、文件变化和必要实时检查尚未逐条排除，因此81不等于81次全部无效，也不能直接推算节省比例。

最近有累计用量的一个会话记录57次请求、input_tokens812250、output_tokens9089、total_tokens821339。这是会话累计，而非前述300次工具样本的精确账单或某次轨迹操作账单。工具调用数、模型请求数、显示行数和计费tokens必须分开。

代码确认：

- terminal/read_cache.py 在模型已经提出调用之后才复用本地结果；terminal/llm.py 的正文引用去重也发生在调用以后。因此能少读磁盘/少重复输出正文，不能消除已经发生的模型请求和工具调用。缓存存在不代表模型无需重新发现证据。
- 未配置 context_window 时，llm.py 以12000字符历史预算调用 context_window.select，并受历史消息条数限制。select按完整对话组选择；最近组本身超预算时会在首组就break，可能不选任何完整历史组。遗漏历史主要回填用户请求摘录，不能恢复全部工具证据。完整历史保存在磁盘不等于模型实际看到了它。
- BASE_PROMPT 要求执行目标先 session_task_update，再定义验收、更新状态；任务注册是显式模型工具调用。样本中10%为该工具，部分确定性记账可设计为执行器事件处理，而不是反复让模型填写。
- 工具接口把读Skill、取得可执行包哈希、执行、状态和日志拆分；输出截断后需分页/sidecar读取。拆分可以提供检查边界，但如果每步都需要新模型往返，就放大成本。应保留底层权限和证据，将确定性操作组合在工具实现中。
- 前台max_tool_rounds=None；存在2轮无证据提示、6轮无进展收束，但调用获得新的内容特征或执行类动作时可能重置停滞计数。因此不能把这称为已经实现可靠的跨调用去重，硬限次数也不能代替进展判断。

比较边界：OpenAI官方《Unrolling the Codex agent loop》（2026-01-23，2026-09-08查阅）同样描述模型→工具→结果→再请求的循环，并明确单轮也可能产生数百工具调用，公开说明shell工具及上下文管理。它没有提供本项目同任务、同模型、同权限的调用次数基线。不能宣称Codex永不重复或推测其隐藏实现。来源：https://openai.com/index/unrolling-the-codex-agent-loop/

建议优先级（未在本轮实施）：先为相同输入建立调用/请求/有效证据/耗时/token账本；把必要任务记账程序化；维护有界的已读证据索引与当前步骤状态，避免压缩后重新发现；将常用执行检查组合为少量有明确回执的工具操作；对同参数同内容且无新状态需求的调用做实际去重。最后用同一批任务、相同模型与权限比较，区分模型决策、工具粒度和执行器额外成本。不用降低安全验收或单纯压低轮数换取表面次数减少。


## 14. 2026-09-08 首轮优化落地

本次直接调整执行器上下文，不增加额外模型请求：

1. 逐轮证据从笼统 received 扩展到实际工具名、path/sha256/行区间、returncode、report/stdout_path/stderr_path，并保留最多1200字符的短正文/输出；历史观察不等于当前状态，预览不完整显式标注。
2. 前台启用最多3轮、6000字符的证据上下文。它可能增加一次请求的固定输入，以减少下一轮重新定位/读取；不能据此宣称总token必然下降。旧摘要缺少的信息不会凭空补齐。
3. 重复读取同路径/哈希/区间在摘要中合并；13条重复读取/引用的受控测试保留为1条完整短正文证据，节省的是摘要条目，不是声称已消除12次真实模型调用。
4. 最新对话组正文超预算时，在消息数量及媒体预算允许的情况下生成有损视图，保留问答和工具配对；避免直接变成空历史。完整持久化记录不修改。
5. 提示沿用已有 active goal，只在范围/验收/状态/下一步实质变化时更新工作记录，不要求重复登记或读回自己的更新。尚未将全部任务登记自动化，仍保留工具与验收门禁。

验证：上下文/读取循环/token预算/workflow/coding/Session恢复53项通过；新增摘要去重与学习边界、原有学习/分层记忆39项通过，两组包含重叠测试。受控下一轮模型输入确实包含已读短文件，无额外文件工具请求；这是可见性测试，不是真实模型A/B调用下降实验。没有调用付费模型或机器人。旧进程重开加载；本次没有扩大工具权限、放宽安全验收或减少实际设备必要检查。
