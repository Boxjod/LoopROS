...
from terminal.model_library import install
...
class ModelLibraryTests(unittest.TestCase):
    ...
    def test_verified_install_offline_cache_and_tamper_rejection(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            with patch('terminal.model_library.catalog', return_value=self.listing(data)), patch('terminal.model_library.fetch', return_value=data):
                result = install(directory, 'test_arm')
                self.assertFalse(result['cached'])
                self.assertTrue(Path(result['scene']).exists())
            with patch('terminal.model_library.catalog', side_effect=AssertionError('Cache must work offline')):
    ...
    def test_bad_download_never_publishes_and_paths_rejected(self):
        data = b'<mujoco/>'
        with tempfile.TemporaryDirectory() as directory:
            with patch('terminal.model_library.catalog', return_value=self.listing(data)), patch('terminal.model_library.fetch', return_value=b'bad'):
                with self.assertRaisesRegex(ValueError, 'checksum'):
                    install(directory, 'test_arm')
    ...
    def test_public_mesh_pinned_download_and_local_reuse(self):
        from terminal.model_library import install_public,resolve
        import mujoco
        data=b'v 0 0 0\nv 0.1 0 0\nv 0 0.1 0\nv 0 0 0.1\nf 1 3 2\nf 1 2 4\nf 1 4 3\nf 2 3 4\n'
        ...
        def fetch(url,**kwargs):
            ...
            return data
        with tempfile.TemporaryDirectory() as directory,patch('terminal.model_library.fetch',side_effect=fetch):
            ...
            with patch('terminal.model_library.catalog',side_effect=AssertionError('local first')):
    ...
    def test_missing_official_model_searches_public_web(self):
        from terminal.model_library import search
        with tempfile.TemporaryDirectory() as directory,patch('terminal.model_library.catalog',return_value={'models':[]}),patch('terminal.household_assets.search',return_value=[]),patch('terminal.web.dispatch',return_value={'results':[{'url':'https://github.com/example/objects/blob/main/cup.obj'}]}) as web:
