    ...
    def check_operator_panel(self, rows, columns):
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                ...
                self.assertEqual(screen.display[prompt + 1], '─' * columns)
                os.write(master, '第一行\n第二行'.encode())
                drain(.2)
                ...
                drain(.4)
                prompt = next(i for i, line in enumerate(screen.display) if line.startswith('❯ '))
                self.assertEqual(screen.display[prompt - 1], '─' * new_columns)
                self.assertEqual(screen.display[prompt + 1], '─' * new_columns)
                os.write(master, b'/exit\r')
                drain(.4)
                self.assertEqual(process.wait(timeout=3), 0)
    ...
    def check_screen(self, rows, columns):
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                ...
                self.assertEqual(screen.display[prompt_row - 1], '─' * columns)
                self.assertEqual(screen.display[prompt_row + 1], '─' * columns)
