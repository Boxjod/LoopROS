...
class WeatherDialogTests(unittest.TestCase):
    ...
    def test_missing_city_question_and_fresh_queries_use_model_tool_loop(self):
        with patch('terminal.app.web_dispatch',side_effect=[{'location':'北京','temperature':29.8},{'location':'天津','temperature':30.2}]) as weather, patch.object(self.app.client,'complete',side_effect=[{'content':'哪个城市？'},call('web_weather',location='北京'),{'content':'北京29.8°C'},call('web_weather',location='天津'),{'content':'天津30.2°C'}]):
    ...
    def test_weather_errors_and_permissions_are_tool_receipts(self):
        for denied in (False,True):
            ...
            with patch('terminal.app.web_dispatch',side_effect=RuntimeError('timeout')) as weather, patch.object(self.app.client,'complete',side_effect=[call('web_weather',location='北京'),{'content':'查询未完成。'}]) as model:
    ...
    def test_weather_coding_and_followup_are_not_intercepted(self):
        ...
        with patch('terminal.app.web_dispatch') as weather, patch.object(self.app.client,'complete',return_value={'content':'Coding response'}) as model:
