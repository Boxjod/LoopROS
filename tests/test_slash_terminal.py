...
class SlashTerminalTests(unittest.TestCase):
    def test_menu_fast_and_chinese_during_stream(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            try:
                send(duration=.6)
                self.assertIn('Ask LoopROS to do anything about Robot', '\n'.join(screen.display))
                placeholder = 'Ask LoopROS to do anything about Robot'
                row = next(i for i, line in enumerate(screen.display) if placeholder in line)
                column = screen.display[row].index(placeholder)
                self.assertEqual(screen.buffer[row][column].fg, '9e9e9e')
                os.write(master, '中文草稿'.encode())
                shown = send(duration=.3)
                self.assertIn('❯ 中文草稿', shown)
                self.assertNotIn(placeholder, shown)
                os.write(master, b'\x15')
                self.assertIn(placeholder, send(duration=.3))
                self.assertIn('tasks 0', '\n'.join(screen.display))
                self.assertNotIn('Session Tokens', '\n'.join(screen.display))
                ...
                os.write(master,b'/swi')
                shown = send(duration=.4)
                self.assertIn('/switch setup', shown)
                self.assertIn('Configure provider', shown)
                ...
                self.assertIn('/model model-b ', '\n'.join(screen.display))
                send('high',.3)
                self.assertIn('Ask LoopROS to do anything about Robot', '\n'.join(screen.display))
                self.assertNotIn('Choose reasoning effort', '\n'.join(screen.display))
                self.assertNotIn('❯ /model', '\n'.join(screen.display))
                send('/fast',.3)
                self.assertIn('priority', '\n'.join(screen.display))
                send('/model model-b high',.4)
                self.assertIn('Ask LoopROS to do anything about Robot', '\n'.join(screen.display))
                send('中',.15)
                ...
                self.assertIn('文2', send(duration=.3))
                os.write(master,b'\x1b[A')
                self.assertIn('❯ 中', send(duration=.3))
